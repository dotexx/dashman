...
restart_dashd(){
    ...
    ok "${messages["done"]}"
    pending " --> dash-cli getnetworkinfo"
    echo
    $DASH_CLI getnetworkinfo
    echo
...
get_dashd_status(){
    ...
    DASHD_SYNCED=0
    if [ $DASHD_CURRENT_BLOCK -ge $CHECK_SYNC_AGAINST_HEIGHT ] && [ $(($CHECK_SYNC_AGAINST_HEIGHT - 5)) -lt $DASHD_CURRENT_BLOCK ];then
        DASHD_SYNCED=1
    fi
...
print_status() {
    ...
    if [ $MN_REGISTERED -gt 0 ] ; then
        ...
        pending " protx payout address        : " ; ok "$MN_PROTX_PAYOUT_ADDRESS | $(get_wallet_balance $MN_PROTX_PAYOUT_ADDRESS) dash"
